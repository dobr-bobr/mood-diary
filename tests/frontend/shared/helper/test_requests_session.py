...
PATCH_TARGET_SESSION_STATE = (
    "mood_diary.frontend.shared.helper.requests_session.st.session_state"
)
...
def test_provide_requests_session_returns_existing_session(mocker):
    mock_session_state = mocker.patch(PATCH_TARGET_SESSION_STATE)
    mock_session_state.__contains__.side_effect = (
        lambda key: key == "requests_session"
    )
