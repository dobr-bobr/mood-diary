...
def create_mood_chart(df):
    line = alt.Chart(df).mark_line(interpolate="monotone", strokeWidth=3).encode(
...
def get_user_ratings_data():
    ...
    for entry in mood_data:
        try:
            ...
            date_obj = (datetime.datetime.fromisoformat(date_str) if "T" in date_str
                        else datetime.datetime.combine(datetime.date.fromisoformat(date_str), datetime.time()))

            ratings.append({"date": date_obj, "rating": value, "comment": note})
...
if "user_ratings_df" not in st.session_state or st.session_state.get("needs_refresh", False):
    refresh_data()
...
with st.form(key="comment_form", clear_on_submit=False, enter_to_submit=False):
    comment = st.text_area("Your thoughts", value=st.session_state.form_comment,
                           placeholder="Type your thoughts here...", height=100)
    ...
    for i, col in enumerate(rating_cols):
        rating_value = i + 1
        color = ["#ef4056", "#f47d2f", "#f8c13a", "#c0d23e", "#8dc63f",
                 "#53a78c", "#3e83c3", "#5465b3", "#6247aa", "#4a357f"][i]
        ...
        with col:
            ...
            if st.form_submit_button(str(rating_value), help=f"Rating {rating_value}"):
    ...
    if submitted:
        if st.session_state.selected_rating is None:
            ...
            if fetch_create_mood(st.session_state.selected_date, st.session_state.selected_rating, comment):
                st.session_state.form_comment = ""
                st.session_state.selected_rating = None
                refresh_data()
                st.rerun()
