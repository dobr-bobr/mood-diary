...
def register():
  ...
  try:
    response = requests.post("https://mood-diary.duckdns.org/api/auth/register", json=payload)
    data = response.json()
...
with st.form("my_form", enter_to_submit=False):
  ...
  if not compare_passwords(password, confirm_password) and len(password) > 0 and len(confirm_password) > 0:
