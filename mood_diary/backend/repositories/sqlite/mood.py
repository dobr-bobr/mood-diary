...
class SQLiteMoodRepository(MoodStampRepository):
	...
	def init_db(self):
		...
		cursor.execute("CREATE INDEX IF NOT EXISTS idx_moodstamps_user_id ON moodstamps (user_id)")
		cursor.execute("CREATE INDEX IF NOT EXISTS idx_moodstamps_date ON moodstamps (date)")
		cursor.execute("CREATE INDEX IF NOT EXISTS idx_moodstamps_value ON moodstamps (value)")
